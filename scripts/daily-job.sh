#!/bin/bash
# 每日完整任务：克隆 -> 生成报告 -> 转HTML -> 更新仪表盘 -> 清理 -> 推送钉钉

# 自动检测项目根目录
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
LOG_FILE="$PROJECT_ROOT/reports/daily-job.log"

echo "======================================" | tee -a $LOG_FILE
echo "$(date '+%Y-%m-%d %H:%M:%S') - 开始每日任务" | tee -a $LOG_FILE
...
if [ -f "$SCRIPT_DIR/send-to-dingtalk.sh" ]; then
    $SCRIPT_DIR/send-to-dingtalk.sh >> $LOG_FILE 2>&1
    echo "   ✅ 钉钉消息已发送" | tee -a $LOG_FILE
else
    echo "   ⚠️  钉钉脚本不存在" | tee -a $LOG_FILE
fi

echo "$(date '+%Y-%m-%d %H:%M:%S') - 每日任务完成" | tee -a $LOG_FILE
echo "======================================" | tee -a $LOG_FILE
